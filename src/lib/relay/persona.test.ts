...
describe('assembleSystemPrompt', () => {
  it('joins the four parts in trunk → grain → rings → coda order', () => {
    const out = assembleSystemPrompt({ trunk: 'TRUNK', grain: 'GRAIN', rings: 'RINGS', coda: 'CODA' });
    ...
    expect(out.indexOf('RINGS')).toBeLessThan(out.indexOf('CODA'));
  });

  it('trims each part so doc trailing whitespace does not bloat the prompt', () => {
    expect(assembleSystemPrompt({ trunk: ' a \n', grain: 'b', rings: 'c', coda: 'd ' })).toBe('a\n\nb\n\nc\n\nd');
  });

  it('throws if any part is missing or blank', () => {
    expect(() => assembleSystemPrompt({ trunk: 'a', grain: 'b', rings: 'c', coda: '  ' })).toThrow(/coda/);
    expect(() => assembleSystemPrompt({ trunk: '', grain: 'b', rings: 'c', coda: 'd' } as never)).toThrow(/trunk/);
  });

  it('maps the four parts to their relay-agent/ filenames in order', () => {
