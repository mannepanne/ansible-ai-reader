 ...
 * session is idle or terminated (or maxPolls is exhausted). Resource creation is NOT done here — the
 * environment/vault/agent exist already; this only runs a session and returns its transcript.
 */
export async function runSession(
  ma: MaClient,
  ids: RunResourceIds,
  stimulus: string,
  opts: { readerId: string; sleep?: (ms: number) => Promise<void>; pollIntervalMs?: number; maxPolls?: number },
