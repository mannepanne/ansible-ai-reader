...
function makeSupabase({
  ...
  const recorded: { update?: Record<string, unknown>; eqs: unknown[][] } = { eqs: [] };
...
describe('approvePiece', () => {
  it('embeds the body then atomically sets approved + slug + embedding, guarded on pending_review', async () => {
    ...
    const rec = (supabase as never as { __recorded: { update: Record<string, unknown>; eqs: unknown[][] } }).__recorded;
    ...
    // guarded: only flips a row that is still pending_review
    expect(rec.eqs).toContainEqual(['id', 'p1']);
    expect(rec.eqs).toContainEqual(['state', 'pending_review']);
  ...
  it('refuses to approve a piece that is not pending_review (no embed, no update)', async () => {
    const supabase = makeSupabase({ piece: { id: 'p1', body: 'x', state: 'approved' } });
    await expect(approvePiece(deps(supabase), { id: 'p1' })).rejects.toThrow(/approved/);
    expect(mockEmbed).not.toHaveBeenCalled();
    expect((supabase as never as { __table: { update: ReturnType<typeof vi.fn> } }).__table.update).not.toHaveBeenCalled();
  });

  it('throws when the guarded update flips nothing (lost the race / already decided)', async () => {
...
describe('rejectPiece', () => {
  it('sets state=rejected + decided_at, guarded on pending_review, without embedding', async () => {
    const supabase = makeSupabase({ updateResult: { id: 'p1' } });
    const out = await rejectPiece(deps(supabase), { id: 'p1' });

    expect(mockEmbed).not.toHaveBeenCalled();
    const rec = (supabase as never as { __recorded: { update: Record<string, unknown>; eqs: unknown[][] } }).__recorded;
    expect(rec.update).toMatchObject({ state: 'rejected' });
    expect(rec.update).not.toHaveProperty('embedding');
    expect(rec.eqs).toContainEqual(['state', 'pending_review']);
    expect(out).toEqual({ ok: true, id: 'p1' });
  });

  it('throws when the piece is missing or not pending_review', async () => {
