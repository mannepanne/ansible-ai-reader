// ABOUT: Interactive demo page showing pre-generated summaries and commentary
// ABOUT: Static content gated by email capture; all interactions are tracked
...
import { useTracking, getStoredEmail } from '@/hooks/useTracking';
...
const demoArticles: DemoArticle[] = [
  ...
  },
  {
    ...
    expandedSummary:
      'The voluntary carbon offset market has contracted by 38% since its 2023 peak, as a cascade of investigative reporting and academic studies revealed that the majority of rainforest preservation credits represented "phantom reductions" — paying to protect forests that were never at risk of being cut down.\n\nThe article traces the collapse through three phases. First, a series of academic papers demonstrated that over 90% of REDD+ credits from major certification bodies like Verra did not represent real emission reductions. Second, major corporate buyers quietly dropped offset claims from their sustainability reports. Third, the certification bodies split over reform proposals.\n\nThe market hasn\'t disappeared entirely. Credits tied to direct carbon removal are trading at 5-10x the price of avoided-deforestation credits. But these represent less than 3% of total offset volume.',
    commentary:
      "The article's central narrative — that carbon offsets were largely a fiction — is well-supported by the evidence but risks throwing out genuine forest conservation finance along with fraudulent credits.\n\nCounter-evidence: While REDD+ credits overestimated their climate impact, many offset-funded projects delivered real biodiversity and community livelihood benefits. A 2025 analysis by the Tropical Ecology Institute found that offset-funded forest concessions in Borneo maintained 40% higher biodiversity than unprotected areas, regardless of whether the carbon math was accurate.\n\nCompeting framework: Some climate economists argue the offset market's collapse is actually healthy creative destruction. The market was functioning as a \"permission to pollute\" mechanism that delayed genuine decarbonization.",
    tags: [
  ...
  },
  {
    ...
    expandedSummary:
      "New York, California, and the EU now require algorithmic impact assessments for AI systems used in employment, housing, and lending decisions. But auditors report that large language models are fundamentally resistant to the kind of systematic bias testing the laws envision — the same prompt can produce different outputs each time.\n\nThe article interviews three firms that have been hired to conduct algorithmic audits under the new regulations. All three describe a fundamental mismatch: the laws were designed for deterministic systems, but the market has moved to probabilistic foundation models where outputs vary with temperature settings, context windows, and even server load.\n\nOne auditing firm ran the same bias test suite against a hiring screening tool 50 times and got statistically significant different results on 23 of those runs.",
    commentary:
      "The article correctly identifies a real gap between regulatory intent and technical reality, but overstates the novelty of the problem. Stochastic systems are common in regulated industries — financial risk models, clinical drug trials, and actuarial calculations all produce variable outputs and have established auditing methodologies.\n\nCounter-evidence: The \"50 runs, 23 different results\" example sounds alarming but lacks context. If the variation is between \"bias score of 0.12 and 0.14,\" the system is functionally consistent even if not deterministically identical. The article doesn't distinguish between meaningful variation and noise.\n\nCompeting framework: Researchers at the Partnership on AI argue that the focus on auditing individual model outputs misses the more important question of systemic outcomes. Rather than testing whether a hiring tool might produce biased results on any given run, regulators should examine whether the tool's aggregate hiring recommendations show demographic disparities.",
    tags: [
...
export default function DemoPage() {
  ...
  // Gate: require email capture before accessing the demo
  useEffect(() => {
  ...
  const visibleArticles = demoArticles.filter((a) => !archivedIds.has(a.id));
    ...
    <div className="min-h-screen bg-[hsl(220,14%,96%)] font-sans">
      ...
      <header className="bg-[hsl(220,15%,18%)] text-white px-4 sm:px-6 py-3 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-gray-400 text-xs hidden sm:inline">demo@ansible.app</span>
        </div>
      </header>
      ...
      <div className="bg-amber-50 border-b border-amber-200 px-4 sm:px-6 py-3 text-center">
        <p className="text-sm text-amber-800">
          You&apos;re viewing a read-only demo with pre-generated summaries and commentary.{' '}
          <span className="font-medium">Expand cards and switch tabs to explore.</span>
        </p>
      </div>
      ...
      <footer className="border-t border-gray-200 bg-white px-4 sm:px-6 py-8 mt-8">
        <div className="max-w-4xl mx-auto text-center">
          <p className="font-serif text-xl mb-2">Like what you see?</p>
          <p className="text-sm text-gray-500 mb-4">
            Ansible is currently in early access. You are on the waitlist to be notified when we
            launch.{' '}
