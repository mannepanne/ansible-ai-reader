...
- **MA surface (largely confirmed against current docs):** remote `mcp_servers: [{type:"url",name,url}]`, **static-bearer credential from a vault** via `vault_ids`, and `environment: cloud` + `limited` networking all exist as specified. Residual build-time check: confirm the harness injects nothing gate-revealing into the agent's visible context (low risk — we own system + tools; promotion is backend-side).
