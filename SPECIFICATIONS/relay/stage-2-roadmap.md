...
**In scope (2.3c — the gate activity log, shipped):** make the gate's *skips* visible. The filter silently dropped no-signal / vetoed archives; now `evaluateRelayTriggers` records its per-item outcome (`relay_gate_code` + `relay_gate_signals`, 2 columns on `reader_items`, written in the existing stamp UPDATE) and the admin **"Activity log"** (renamed from "Decision log") shows gate skips — *not reacted*, with the reason and the signals present — merged chronologically with the WROTE/DECLINED session decisions. Reshaped from a dedicated table to columns after `/review-spec` (dissolved a duplicate-session bug + two open questions). Full detail: [stage-2.3c-gate-activity-log-spec.md](./stage-2.3c-gate-activity-log-spec.md).
