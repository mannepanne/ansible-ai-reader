...
**2.2a DoD §G — hand-distillation pass AUTHORED (loop closed at the authoring layer):** the first genuine capture (an approve-with-note on the `debottlenecking` piece flagging *"Watch the order of operations, because it gives the game away."* as an AI tell) was distilled into a new **"The knowing nudge"** ✗/✓ anti-tell in `craft-and-cadence.md`. The capture arrived through the real approve-with-note path, so the 2.2a capture mechanism is demonstrably working. Honest caveats: the signal is **thin** — 1 note across 18 pieces (4 approved / 14 rejected), and **0 edit-deltas** (`original_body` has never been written by real data, so the edit-capture half of 2.2a is merged-but-unexercised); the note called the tell *"fine to overlook"*, so cataloguing it under "cut them" is a deliberate, slightly-firmer generalisation. **Still owed for the loop to close end-to-end (operator, not code):** (1) **deploy** — `npm run relay:session <reader_id>` re-pushes the assembled prompt to the pinned agent `RELAY_AGENT_ID` (the doc edit is inert until then — the prod DO orchestrator runs a pinned agent and never re-assembles); (2) **demonstrate** — confirm a fresh run no longer emits the nudge tell. Ongoing note/edit discipline on every review is what makes 2.2b (the automated distiller) worth building.
