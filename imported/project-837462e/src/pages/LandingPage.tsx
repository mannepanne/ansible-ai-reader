...
import { captureEmail, setSessionEmail, getStoredEmail, verifyStoredEmail, usePageTracking } from "@/hooks/useTracking";
...
function Navbar({ trackPageEvent }: { trackPageEvent: (type: string, data?: Record<string, unknown>) => void }) {
    ...
    <nav className="sticky top-0 z-50 border-b border-gray-100 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-6">
          <button
            onClick={() => { scrollTo("features"); trackPageEvent("nav_click", { target: "features" }); }}
            className="text-sm text-muted-foreground hover:text-foreground transition-colors"
          >
            Features
          </button>
          <button
            onClick={() => { scrollTo("how-it-works"); trackPageEvent("nav_click", { target: "how-it-works" }); }}
            className="text-sm text-muted-foreground hover:text-foreground transition-colors"
          >
            How it works
          </button>
...
function NoiseField() {
  const particles = useMemo(() => {
    const items: { x: number; y: number; size: number; opacity: number; delay: number; duration: number }[] = [];
    // Dense field at top, dramatically thinning toward bottom
    for (let i = 0; i < 500; i++) {
      // Strong weighting toward top: pow(rand, 2.5) clusters heavily near y=0
      const weightedY = Math.pow(Math.random(), 2.5);
      // Top particles: high opacity, nearly solid. Bottom: very faint
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
      ...
      {/* Fade to white at bottom for clean transition */}
...
function useVerifiedEmail() {
  const [hasSubmitted, setHasSubmitted] = useState(() => !!getStoredEmail());
...
function ProductPreview() {
  ...
  const visibleArticles = previewArticles.filter(
    ...
    <section id="features" className="py-20 px-6">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16">
          <p className="font-serif italic text-gray-400 mb-3">
            Three lenses on every article
          </p>
  ...
  const steps = [
    {
      ...
      description:
        "Use Readwise Reader as you normally do. Save articles, newsletters, podcasts — anything you want to read later.",
    },
    {
      icon: Filter,
      step: "02",
      ...
      description:
        "AI generates summaries and commentary for your unread items. Colored tags surface key topics at a glance.",
    },
    {
      icon: Clock,
      step: "03",
      ...
      description:
        "Skim the summaries. Check the critique. Open only the pieces that deserve your full attention.",
    },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((step) => (
            <div key={step.step} className="text-center">
              <div className="font-serif text-3xl text-gray-300 mb-4">
                {step.step}
              </div>
    ...
    <section className="py-20 px-6">
      <div className="max-w-2xl mx-auto text-center">
        ...
        <p className="text-sm text-gray-400 font-sans">
          Ursula K. Le Guin,{" "}
          <span className="italic">The Dispossessed</span>
        </p>
        ...
        {hasSubmitted ? (
          <>
            <p className="text-muted-foreground mb-8">
              See how Ansible triages your reading queue with AI summaries and
              commentary.
            </p>
            <Button
              size="lg"
          ...
          <>
            <p className="text-muted-foreground mb-8">
              Enter your email to access the interactive demo and see Ansible in
              action.
            </p>
            <form
              onSubmit={handleSubmit}
