...
**The admin "Relay Agent" tab is now the primary surface** (Admin → Analytics Dashboard, behind the `is_admin` login): a **"Run a session"** control triggers a run, and pending/approved/rejected pieces can be reviewed and approved/rejected (incl. re-decisions) with a paginated **"Activity log"**. The Activity log (Stage 2.3c) merges, newest-first, the narrator's session decisions (`WROTE` / `DECLINED`) with the engagement gate's **skips** (`NOT REACTED` — an archived item the gate never woke the narrator for, shown with the skip reason and the signals that were present or overridden). When the gate is off it says so, so an empty skip section isn't misread as "nothing skipped". Gate pass / not-reacted counts sit alongside the decision-verdict widgets. Each piece card also shows a **Reader deep-link** to the source article and a collapsible **reconstructed stimulus** (what the piece was written from). The stimulus is *reconstructed* with today's `formatStimulus`, not the exact text sent — a piece written before an assembler change (e.g. pre-2.3a `Tags`/`Note`) renders lines it never saw, so the UI labels it "reconstructed (current logic)". Approve/Reject/Run proxy to the backend via admin-gated routes (`/api/admin/relay/review`, `/api/admin/relay/run`) using `RELAY_CONTROL_TOKEN` / the `RELAY_ORCHESTRATOR` Durable Object binding.

**How the tab trigger runs a session (vs. the CLI):** the tab hands a `reader_id` to the `RelayOrchestrator` Durable Object (via the `RELAY_ORCHESTRATOR` binding); the DO runs one session at a time (serial by construction — no lock table) and finalizes via the bridge. See [ADR 2026-07-07](../decisions/2026-07-07-relay-orchestrator-durable-object.md), which superseded the earlier queue-consumer trigger ([ADR 2026-07-01](../decisions/2026-07-01-relay-session-trigger.md)). The trigger works **in production only** (the DO binding isn't available under local `next dev`). The `relay:session` CLI below remains the way to run sessions locally, to update the agent's voice, and as a scriptable alternative.
