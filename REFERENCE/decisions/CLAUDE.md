...
- **[2026-09-12-plain-review-output.md](./2026-09-12-plain-review-output.md)** — Why review output from `/review-spec`, `/review-pr` and `/review-pr-team` is budgeted in lines (verdict first, one to three lines per finding, praise capped at three sentences, no per-reviewer count block); why the writing rule in `.claude/COLLABORATION/writing-style.md` defines mannered prose as a mechanism with examples instead of a banned-word list; and why the essentials are repeated inline in `.claude/CLAUDE.md` and the agent contract rather than living only behind a pointer.
...
- **[2026-09-07-typed-supabase-client.md](./2026-09-07-typed-supabase-client.md)** — Why every Supabase client carries the generated `Database` type: the compiler catches the ambiguous-embed bug behind PR #146 and the nullability gaps the untyped client hid; the procedural rule (regenerate after every migration, no CI check), the pgvector and Json casts, and why the real-schema replay is still needed.
